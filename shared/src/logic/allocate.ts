...
//
// Two operations:
//   spendPoints  — incrementally bump one stat by `amount` (server-validated
//                  via the same code path so cheating is impossible).
//   resetStats   — reset all 6 primary stats to STAT_BASE and refund every
//                  spent point as unspentPoints. Triggered by item / admin.
import type { GameState, PrimaryStat } from '../types.js'
